...
const {
  ...
  getState: () => ({
    ...
    templatePlaceholderValues: { ...templatePlaceholderValues.value },
  ...
  onRestore: (data) => {
    ...
    if (data.templatePlaceholderValues) templatePlaceholderValues.value = data.templatePlaceholderValues
...
const templatePlaceholders = computed(() => {
  ...
  try {
    const fields = typeof selectedTemplate.value.placeholder_fields === 'string'
      ? JSON.parse(selectedTemplate.value.placeholder_fields)
      : selectedTemplate.value.placeholder_fields
...
// Only show input-worthy placeholders (exclude 'image' — that's handled separately)
const inputPlaceholders = computed(() => {
  return templatePlaceholders.value.filter(p => p !== 'image' && p !== 'bild')
})

// Check if all placeholders have been filled
const placeholdersFilled = computed(() => {
  if (inputPlaceholders.value.length === 0) return true
  return inputPlaceholders.value.every(p => {
    const val = templatePlaceholderValues.value[p]
    return val && val.trim().length > 0
  })
})

// Count of unfilled placeholders
const unfilledPlaceholderCount = computed(() => {
  if (inputPlaceholders.value.length === 0) return 0
  return inputPlaceholders.value.filter(p => {
    const val = templatePlaceholderValues.value[p]
    return !val || val.trim().length === 0
  }).length
})

// Get display info for a placeholder
function getPlaceholderInfo(fieldName) {
...
watch(selectedTemplate, (tmpl) => {
  ...
  const newValues = {}
  try {
    const fields = typeof tmpl.placeholder_fields === 'string'
      ? JSON.parse(tmpl.placeholder_fields)
      : tmpl.placeholder_fields
    if (Array.isArray(fields)) {
      for (const field of fields) {
        // Preserve existing values if switching between similar templates
        newValues[field] = templatePlaceholderValues.value[field] || ''
      }
    }
  } catch { /* ignore */ }
  templatePlaceholderValues.value = newValues
})
...
<template>
  <div class="max-w-[90rem] mx-auto">
    ...
    <div v-if="currentStep === 2">
      ...
      <div
        v-if="selectedTemplate && inputPlaceholders.length > 0"
        class="mt-6 p-5 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-750 border border-blue-200 dark:border-blue-800 rounded-xl"
        data-testid="placeholder-inputs"
      >
        <div class="flex items-center justify-between mb-4">
          ...
          <span
            ...
            class="text-xs font-medium px-2.5 py-1 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded-full"
          >
            {{ unfilledPlaceholderCount }} offen
          </span>
          <span
            v-else
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div
            v-for="field in inputPlaceholders"
            :key="field"
            class="relative"
          >
            <label
            ...
            <input
              :id="'ph-' + field"
              v-model="templatePlaceholderValues[field]"
              type="text"
              :placeholder="getPlaceholderInfo(field).desc"
              class="w-full px-3 py-2 text-sm rounded-lg border bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-[#3B7AB1] focus:border-transparent transition-colors"
              :class="templatePlaceholderValues[field]?.trim()
