...
const categoryOptions = [
  ...
  { value: 'country', label: 'Länderbild' },
...
async function uploadFile(file) {
  ...
  if (file.size > maxSize) {
    uploadError.value = `Datei ist zu groß (max. ${maxLabel})`
    return
  }
      ...
      <div v-else-if="stockResults.length > 0" data-testid="stock-results">
        <div class="flex items-center justify-between mb-3">
          <p class="text-sm text-gray-600 dark:text-gray-400">
            {{ stockResults.length }} Ergebnis{{ stockResults.length !== 1 ? 'se' : '' }} für "{{ stockSearchQuery }}"
    ...
    >
      ...
      <div>
        ...
        <p class="text-xs text-blue-600 dark:text-blue-400 mt-0.5">
          Deine Assets stehen automatisch im Post-Editor (Schritt 5: Bild), im Video-Composer, Video-Export und Thumbnail-Generator zur Verfügung. Tagge Assets mit Land und Kategorie für automatische Filterung.
        </p>
      </div>
    </div>
