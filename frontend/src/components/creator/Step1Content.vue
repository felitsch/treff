...
const categories = [
  ...
  { id: 'story_teaser', label: 'Story-Teaser', icon: 'paper-airplane' },
  { id: 'story_series', label: 'Story-Serien', icon: 'book-open' },
]
...
const toneOptions = [
  ...
  { id: 'informativ', label: 'Informativ', icon: 'chart-bar' },
  { id: 'storytelling', label: 'Storytelling', icon: 'book-open' },
]
  ...
  <div class="space-y-6" data-testid="step1-content">
    ...
    <div>
      ...
      <div class="flex flex-wrap gap-2">
        ...
        >
          <AppIcon :name="cat.icon" class="w-4 h-4 inline-block mr-1" />{{ cat.label }}
        </button>
      </div>
      <!-- Content Pillar badge (compact indicator) -->
    ...
    <!-- Content Pillar Selector (override auto-assignment) -->
    <div>
      <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
        Content Pillar
        <span class="font-normal text-gray-400 text-xs ml-1">(optional — wird automatisch aus Kategorie abgeleitet)</span>
      </label>
      <ContentPillarSelector
        v-model="selectedPillar"
        :compact="true"
        data-testid="step1-pillar-selector"
      />
    </div>
    ...
    <div>
      <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
        Buyer Journey Phase
        <span class="font-normal text-gray-400 text-xs ml-1">(beeinflusst KI-Text und Hook-Formeln)</span>
      </label>
      <BuyerJourneySelector
        v-model="selectedBuyerJourneyStage"
        :selected-pillar="selectedPillar"
        data-testid="step1-journey-selector"
      />
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <div class="flex flex-wrap gap-2">
          ...
          >
            <AppIcon :name="t.icon" class="w-4 h-4 inline-block" /> {{ t.label }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-if="successMsg" class="p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg text-green-700 dark:text-green-300 text-sm flex items-center gap-2">
      <AppIcon name="check-circle" class="w-4 h-4" /> {{ successMsg }}
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-6 hover:border-[#3B7AB1] transition-colors">
        <div class="flex justify-center mb-2"><AppIcon name="camera" class="w-8 h-8 text-gray-400" /></div>
      ...
      <div class="border-2 border-purple-200 dark:border-purple-700 rounded-xl p-6 bg-purple-50/30 dark:bg-purple-900/10">
        <div class="flex justify-center mb-2"><AppIcon name="sparkles" class="w-8 h-8 text-purple-500" /></div>
        ...
        >
          ...
          <span v-else class="inline-flex items-center gap-1"><AppIcon name="sparkles" class="w-4 h-4" /> Bild generieren</span>
        </button>
